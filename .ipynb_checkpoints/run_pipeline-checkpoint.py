...
Commands:
    ...
    all       — Run phase2 → phase3 → tuning → aligned → phase4 → trading → backtest
...
# Pipeline order (phase1 is optional, run separately or via 'full')
PIPELINE_SCRIPTS = [
    "step2_feature_engineering.py",
    "step3_model_training.py",
    "step3b_hyperparameter_tuning.py",
...
def run_full_pipeline(include_phase1: bool = False) -> int:
...
def main() -> int:
    ...
    if args.command == "full":
        return run_full_pipeline(include_phase1=True)

    # All (skip phase1)
    return run_full_pipeline(include_phase1=False)
