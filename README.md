...
- **Classifier:** `RandomForestClassifier(max_depth=3, random_state=42)` — chosen for small-dataset robustness and fair comparison
- **Scaling:** `StandardScaler` fitted only on training data per model (prevents data leakage)
- **Split:** Chronological only (no shuffling) — respects time-series nature of financial data
- **Sentiment scoring:** VADER compound score → discrete labels (-1, 0, +1) with category-weighted impact
- **Test set:** Last 10 trading days, shared across all models (Universal Test Set)
- **Training windows:** Model A uses full 5yr history; Models AA and B use 25 days before test set
